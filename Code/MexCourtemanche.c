...
/* Mex Function*/
static void MexCourtemanche(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double alpha_y1 = (P3*(v+P1))/(1-(exp(-P1*P4)*exp(-P4*v)));
    const double beta_y1 = P5*(v-P6)/((exp(-P6*P7)*exp(P7*v))-1);
    
    
    
    const double y1_inf = 1/(1+(exp(-P1*P2)*exp(-P2*v)));
    const double tau_y1 = 1/(alpha_y1+beta_y1);
        ...
        
{
    ...
    /* Mex function call */
    MexCourtemanche(ydot, t, y, pr, T, Y0, M, yout);
    return;
