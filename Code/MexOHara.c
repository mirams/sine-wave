...
/* Mex Function*/
static void MexOHara(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout, double* zout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double tau_fast = P3 + (1/((P4*exp(P5*v) + P6*exp(-P7*v))));
    
    const double tau_slow = P10 + (1/((P11*exp(P12*v) + P13*exp(-P14*v))));
    const double y1_inf =1/(1+P8*exp(-P9*v));
        ...
        
{
    ...
    /* Mex function call */
    MexOHara(ydot, t, y, pr, T, Y0, M, yout, zout);
    return;
