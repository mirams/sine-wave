...
/* Mex Function*/
static void MexRamirez(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double alpha_y1 = (P1*(v-P2))/(1-(exp(P2*P3)*exp(-P3*v)));
    const double beta_y1 = (P4*(v+P5))/((exp(P5*P6)*exp(P6*v))-1);
    
    
    
    const double y1_inf = 1/(1+P7*exp(-P8*v));
        ...
        
{
    ...
    /* Mex function call */
    MexRamirez(ydot, t, y, pr, T, Y0, M, yout);
    return;
