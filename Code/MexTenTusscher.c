...
/* Mex Function*/
static void MexTenTusscher(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout, double* zout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double alpha_1 = P1/(1+P2*exp(-P3*v));
    const double beta_1 = 1/(1+P4*exp(P5*v));
    
    const double y1_inf = 1/(1+P6*exp(-P7*v));
    
    const double alpha_2 = P8/(1+P9*exp(-P10*v));
    const double beta_2 = 1/(1+P9*exp(P10*v));
    
    const double y2_inf = 1/(1+P11*exp(P12*v));
    
    
    const double tau_1 = alpha_1*beta_1;
    const double tau_2 = alpha_2*beta_2;
    
    
    NV_Ith_S(ydot, 0) =  (y1_inf- y1)/tau_1;
    NV_Ith_S(ydot, 1) =  (y2_inf- y2)/tau_2;
        ...
        
{
    ...
    /* Mex function call */
    MexTenTusscher(ydot, t, y, pr, T, Y0, M, yout, zout);
    return;
