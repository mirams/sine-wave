...
/* Mex Function*/
static void MexShannon(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double alpha_y1 = (P3*(v+P4))/(1-(exp(-P5*P4)*exp(-P5*v)));
    const double beta_y1 = P6*(v+P7)/((exp(P7*P8)*exp(P8*v))-1);
        ...
        
{
    ...
    /* Mex function call */
    MexShannon(ydot, t, y, pr, T, Y0, M, yout);
    return;
