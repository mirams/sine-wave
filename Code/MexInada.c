...
/* Mex Function*/
static void MexInada(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout, double* zout, double* wout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double tau_slow = P7 + P8*exp(-(P10*v*v)-P9*v);
    const double y3_inf = (1/(1+P11*exp(P12*v)))*(1-P13*exp(-P14*v*v));
    const double alpha_rkr = P15*exp(-P16*v);
    const double beta_rkr = P17*exp(P18*v);
    ...
    NV_Ith_S(ydot, 2) =  (y3_inf- y3)/tau_rkr;
        ...
        
{
    ...
    wout = mxGetPr(plhs[2]);
    /* Mex function call */
    MexInada(ydot, t, y, pr, T, Y0, M, yout, zout, wout);
    return;
