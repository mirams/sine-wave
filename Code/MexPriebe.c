...
/* Mex Function*/
static void MexPriebe(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double alpha = (P1*exp(P2*v))/(1+(exp(-P3*4.067))*exp(-P3*v));
    const double beta = (P4*exp(P5*v))/(1+(exp(P6*65.66))*exp(P6*v));
    
    
    
    NV_Ith_S(ydot, 0) =  alpha*(1-y1)-beta*y1;
        ...
        
{
    ...
    /* Mex function call */
    MexPriebe(ydot, t, y, pr, T, Y0, M, yout);
    return;
