...
/* Mex Function*/
static void MexFox(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double tau_y1 = P1 + (1/((P2*exp(P3*v) + P4*exp(-P5*v))));
        ...
        
{
    ...
    /* Mex function call */
    MexFox(ydot, t, y, pr, T, Y0, M, yout);
    return;
