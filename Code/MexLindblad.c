...
/* Mex Function*/
static void MexLindblad(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout, double* zout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double tau_1 = (1/((P1*exp(P2*v) + P3*exp(-P4*v))));
    const double y1_inf =1/(1+P5*exp(-P6*v));
    
    const double alpha_rkr = P7*exp(-P8*v);
    const double beta_rkr = P9*exp(P10*v);
    const double tau_rkr = 1/(alpha_rkr+beta_rkr);
    const double y2_inf = alpha_rkr/(alpha_rkr+beta_rkr);
    
    
    NV_Ith_S(ydot, 0) =  (y1_inf- y1)/tau_1;
    NV_Ith_S(ydot, 1) =  (y2_inf- y2)/tau_rkr;
        ...
        
{
    ...
    /* Mex function call */
    MexLindblad(ydot, t, y, pr, T, Y0, M, yout, zout);
    return;
