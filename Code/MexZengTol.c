...
/* Mex Function*/
static void MexZengTol(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    /* Normal tau calculation */
    
    
    
    if (((v+P7)<tol)||(v+P7)>-tol){
        /* Close to V = -E singularity */
        tau_y1 = 1.0/(P3*(v+P4)/(1.0-exp(-P5*(v+P4))) + P6/P8 -0.5*P6*(v+P7));
    }
    
    if (((v+P4)<tol)||(v+P4)>-tol){
        /* Close to V = -B singularity*/
        tau_y1  = 1.0/((P3/P5) + 0.5*P3*(v+P4) + P6*(v+P7)/(exp(P8*(v+P7))-1.0));
    }
    
    
    if ((((v+P4)>tol)||((v+P4)<-tol))&&(((v+P7)>tol)||((v+P7)<-tol))){
        
        
        tau_y1 = 1.0/(P3*(v+P4)/(1.0-exp(-P5*(v+P4))) + P6*(v+P7)/(exp(P8*(v+P7))-1.0));
    }
    
    NV_Ith_S(ydot, 0) =  (y1_inf- y1)/tau_y1;
    return 0;
}
        ...
        
{
    ...
    /* Mex function call */
    MexZengTol(ydot, t, y, pr, T, Y0, M, yout);
    return;
