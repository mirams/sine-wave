...
/* Mex Function*/
static void MexKurata(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout, double* zout, double* wout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double y3_inf = (1/(1+P11*exp(P12*v)));
    const double alpha_rkr = P13*exp(-P14*v);
    const double beta_rkr = P15*exp(P16*v);
        ...
        
{
    ...
    wout = mxGetPr(plhs[2]);
    /* Mex function call */
    MexKurata(ydot, t, y, pr, T, Y0, M, yout, zout, wout);
    return;
