...
/* Mex Function*/
static void MexClancy(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double k51 = P10*exp((P1*v));
    const double k15 = P11*exp(-(P12*v));
    ...
    const double k24 = k23;
    
    phi = (k32*k43)/k34;
    
    const double k42 = phi;
    
    NV_Ith_S(ydot, 0) = -k12*y1 + k21*y2 + k51*y5 - k15*y1;
    NV_Ith_S(ydot, 1) = +k12*y1 - k21*y2 - k23*y2 + k32*y3 -k24*y2 + k42*y4;
    NV_Ith_S(ydot, 2) = +k23*y2 - k32*y3 - k34*y3 + k43*y4;
    NV_Ith_S(ydot, 3) = -k43*y4 + k34*y3 - phi*y4 + k24*y2;
        ...
        
{
    ...
    /* Mex function call */
    MexClancy(ydot, t, y, pr, T, Y0, M, yout);
    return;
