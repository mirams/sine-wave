...
/* Mex Function*/
static void MexDiVeroliRT(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double y5 = NV_Ith_S(y, 4);
    const double y6 = (1.0-y1-y2-y3-y4-y5);
    /* Model equations*/
    
    const double k12 = P8*exp(P9*v);
    const double k21 = P10*exp(-P11*v);
    ...
    const double k54 = k23;
    const double k45 = k32;
    
    const double k34 = P4*exp(P5*v);
    const double k43 = P6*exp(-P7*v);
    
    const double k25 = k34;
    const double k52 = k43;
    
    const double k65 = k12;
    const double k56 = k21;
    
    const double k16 = k34;
    const double k61 = k43;
    
    NV_Ith_S(ydot, 0) = -k12*y1 + k21*y2 + k61*y6 - k16*y1;
    NV_Ith_S(ydot, 1) = -k23*y2 + k32*y3 + k12*y1 - k21*y2 -k25*y2 + k52*y5;
    NV_Ith_S(ydot, 2) = -k34*y3 + k43*y4 + k23*y2 - k32*y3;
    NV_Ith_S(ydot, 3) = +k34*y3 - k43*y4 - k45*y4 + k54*y5;
    NV_Ith_S(ydot, 4) = -k54*y5 + k45*y4 - k56*y5 + k65*y6 + k25*y2 - k52*y5;
        ...
        
{
    ...
    /* Mex function call */
    MexDiVeroliRT(ydot, t, y, pr, T, Y0, M, yout);
    return;
