...
/* Mex Function*/
static void MexDiVeroliPT(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double k12 = P1+P2*exp(-(P4*v*v)+P3*v);
    const double k21 = P5+P6*exp(-(P8*v*v)-P7*v);
    
    const double k23 = P8+P9*exp(-(P12*v*v)+P11*v);
    const double k32 = P13+P14*exp(-(P16*v*v)-P15*v);
    
    const double k14 = k23;
    const double k41 = k32;
    
    const double k43 = k12;
    const double k34 = k21;
    
    
    
    
    NV_Ith_S(ydot, 0) = -k12*y1 + k21*y2 -k14*y1 + k41*y4;
    NV_Ith_S(ydot, 1) = +k12*y1 - k21*y2 + k32*y3 - k23*y2;
        ...
        
{
    ...
    /* Mex function call */
    MexDiVeroliPT(ydot, t, y, pr, T, Y0, M, yout);
    return;
