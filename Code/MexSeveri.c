...
/* Mex Function*/
static void MexSeveri(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout, double* zout, double* wout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double tau_fast = (1/((P3*exp(P4*v) + exp(-P5*v))));
    const double tau_slow = (1/((P6*exp(P7*v) + P8*exp(-P9*v))));
    const double tau_rkr = (1/((P10*exp(-P11*v) + P12*exp(P13*v))));
    const double rkr_inf = 1/(1+P14*exp(P15*v));
        ...
        
{
    ...
    wout = mxGetPr(plhs[2]);
    /* Mex function call */
    MexSeveri(ydot, t, y, pr, T, Y0, M, yout, zout, wout);
    return;
