...
/* Mex Function*/
static void MexMatsuoka(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout, double* zout, double* wout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double alpha_fast = (1/((P1*exp(-P2*v) + P3*exp(-P4*v))));
    const double beta_fast = (1/((P5*exp(P6*v) + P7*exp(P8*v))))+ (1/(P9*exp(P10*v)));
    const double alpha_slow = (1/((P7*exp(-P11*v) + P1*exp(-P4*v))));
    const double beta_slow = (1/((P12*exp(P6*v) + P13*exp(P8*v))))+ (1/(P14*exp(P10*v)));
    const double alpha_rkr = (1/((P15*exp(P16*v) + P17*exp(P4*v))));
    const double beta_rkr = (1/((P18*exp(-P16*v) + P19*exp(-P20*v))));
    
    
    
    
    NV_Ith_S(ydot, 0) =  alpha_fast*(1-y1) - beta_fast*y1;
    NV_Ith_S(ydot, 1) =  alpha_slow*(1-y2) - beta_slow*y2;
    NV_Ith_S(ydot, 2) =  alpha_rkr*(1-y3) - beta_rkr*y3;
        ...
        
{
    ...
    wout = mxGetPr(plhs[2]);
    /* Mex function call */
    MexMatsuoka(ydot, t, y, pr, T, Y0, M, yout, zout, wout);
    return;
