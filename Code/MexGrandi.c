...
/* Mex Function*/
static void MexGrandi(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
v=pr[l+10];
    ...
    const double tau_y1 =  (P1/(1+P2*exp(-P3*v)))*(1/(1+P4*exp(v*P3)))+(P5/(1+P6*exp(v*P7)));
        ...
        
{
    ...
    /* Mex function call */
    MexGrandi(ydot, t, y, pr, T, Y0, M, yout);
    return;
