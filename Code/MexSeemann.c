...
/* Mex Function*/
static void MexSeemann(N_Vector ydot, realtype t, N_Vector y, double* pr, double* T, double* Y0, int M, double* yout) {
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* pr0) {
    ...
    const double alpha = P1*exp(P2*v);
    const double beta = P3*exp(-P4*v);
        ...
        
{
    ...
    /* Mex function call */
    MexSeemann(ydot, t, y, pr, T, Y0, M, yout);
    return;
