...
for i in [0,1,2,3,4,6]:
    ...
    mean_error_2[i]  = numpy.sqrt(numpy.mean(pow(currents[:,i]-currents[:,5],2)))

for i in [0,1,2,3,4,6]:
    print(model_names[i],' mean error = ',mean_error[i],' nA')
    print(model_names[i],' mean error 2 = ',mean_error_2[i],' nA')
    print(model_names[i],' percent increase over New Model Error = ',100*mean_error[i]/mean_error[6]-100,'%')
plt.rc('text', usetex=True)
plt.rc('font', family='serif')
plt.tight_layout()
plt.savefig('errors_wang.pdf', bbox_inches='tight', dpi=900, pad_inches=0.05)
...
plt.rc('font', family='serif')
plt.tick_params(axis='both', which='major', labelsize=16)
plt.savefig('figure_5_wang.pdf', bbox_inches='tight', dpi=900, pad_inches=0.05)
