...
image = plt.imread('wang.png')
im = ax5.imshow(image)
ax5.axis('off')

# Histograms of MCMC results
gs3 = gridspec.GridSpec(4, 4, top=0.73, bottom=0.0,left=0.67,right=1.0)
...
ax10 = fig.add_subplot(gs3[1,0],sharey=ax9)
...
ax11 = fig.add_subplot(gs3[1,1],sharey=ax9)
...
ax13 = fig.add_subplot(gs3[1,3],sharey=ax12)
...
ax14 = fig.add_subplot(gs3[2,0],sharey=ax12)
...
ax15 = fig.add_subplot(gs3[2,1],sharey=ax12)
weights = numpy.ones_like(data[:,10])/float(len(data[:,10]))
ax15.hist(data[:,10], 20, normed=0, weights=weights, facecolor='r', edgecolor = "none", alpha=0.75)
ax15.set_xlabel('$P_{10}$')

ax16 = fig.add_subplot(gs3[2,2],sharey=ax12)
weights = numpy.ones_like(data[:,11])/float(len(data[:,11]))
ax16.hist(data[:,11], 20, normed=0, weights=weights, facecolor='r', edgecolor = "none", alpha=0.75)
ax16.set_xlabel('$P_{11}$')

ax17 = fig.add_subplot(gs3[2,3],sharey=ax12)
weights = numpy.ones_like(data[:,12])/float(len(data[:,12]))
ax17.hist(data[:,12], 20, normed=0, weights=weights, facecolor='r', edgecolor = "none", alpha=0.75)
ax17.set_xlabel('$P_{12}$')

ax18 = fig.add_subplot(gs3[3,0],sharey=ax12)
weights = numpy.ones_like(data[:,13])/float(len(data[:,13]))
ax18.hist(data[:,13], 20, normed=0, weights=weights, facecolor='r', edgecolor = "none", alpha=0.75)
ax18.set_xlabel('$P_{13}$')
ax18.set_ylabel('Probability')
ax18.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))

ax19 = fig.add_subplot(gs3[3,1],sharey=ax12)
weights = numpy.ones_like(data[:,14])/float(len(data[:,14]))
ax19.hist(data[:,14], 20, normed=0, weights=weights, facecolor='r', edgecolor = "none", alpha=0.75)
ax19.set_xlabel('$P_{14}$')

ax20 = fig.add_subplot(gs3[3,2],sharey=ax12)
weights = numpy.ones_like(data[:,15])/float(len(data[:,15]))
ax20.hist(data[:,15], 20, normed=0, weights=weights, facecolor='r', edgecolor = "none", alpha=0.75)
ax20.set_xlabel('$G_{Kr}$')
ax20.set_ylim([0., .2])
...
axes_list = [ax6,ax7,ax8,ax9,ax10,ax11,ax12,ax13,ax14,ax15,ax16,ax17,ax18,ax19,ax20]
for i in range(0,len(axes_list)):
    ...
    if i>9:
        ax.get_xaxis().set_label_coords(+0.2,0.9)
    else:
        ax.get_xaxis().set_label_coords(+0.2,0.9)
...
########################################################
left_shift_for_panel_labels = -0.165
...
ax6.text(-0.75, 1.4, 'C', verticalalignment='top', horizontalalignment='left',
...
plt.savefig('figure_3_wang.pdf', bbox_inches='tight', dpi=900, pad_inches=0.05)
